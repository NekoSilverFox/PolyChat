...
|      | ut_login_btnlogin                | Login                 | btnlogin является отмечаемой кнопкой (Checkable-QToolButton), а отображение icon отключено. |
...
|      | ut_login_btnInfo_emit            | Login                 | Каждый раз, когда пользователь нажимает на btnInfo, сигнал (нажатие кнопки) гарантированно срабатывает правильно, и один раз |
...
|      | ut_login_window                  | Login                 | Размер Login отключен и составляет 400x250                   |
...
|      | ut_chatlist_btnNewChat           | ChatList              | btnNewChat является отмечаемой кнопкой (Checkable-QToolButton), а отображение текста отключено. |
...
|      | ut_tcpclient_btnCancel           | TcpClient             | btnCancel является отмечаемой кнопкой (Checkable-QToolButton) |
...
|      | ut_tcpclient_btnSave             | TcpClient             | btnSave является отмечаемой кнопкой (Checkable-QToolButton)  |
...
| 35   | ut_tcpclient_closeEvent_emit     | TcpClient             | Запускает событие закрытия, когда пользователь закрывает окно. |
|      | ut_tcpclient_textBrowser         | TcpClient             | textBrowser доступен только для чтения, запись запрещена.    |
...
|      | ut_tcpserver_btnCancel           | TcpServer             | btnCancel является отмечаемой кнопкой (Checkable-QToolButton) |
...
|      | ut_tcpserver_textBrowser         | TcpServer             | textBrowser позволяет писать.                                |
...
|      | ut_chatbox_tbUser                | ChatBox               | Ширина списка пользователей составляет 150 - 350             |
|      | ut_chatbox_lbNumberOnlineUse     | ChatBox               | UI инициализация онлайн-расчета численности в правильный формат |
