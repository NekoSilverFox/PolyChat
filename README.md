...
| ChatWidget_Mess_Clean        | Нажатие на кнопку "Очистить чат" очистит все журналы чата в свое окне, содержимое окон других пользователей не затрагивается |
...
Эта программа реализует следующие функции: Если каждый пользователь в локальной сети входит в программу чата, то в правой части интерфейса программы может отображаться список онлайн пользователей, которые отображаются как имя пользователя, номер группы, имя хоста и ip-адрес соответственно. Слева от формы находится экран отображения содержимого чата, т.е. группового чата. Каждый пользователь может ввести текст (и изменить формат текста) и отправить его с экрана ввода чата.

### Сервер

После создания UDP-сокета и привязки его к фиксированному порту слушайте входящие данные, используя сигналы и слоты. Если да, он получает данные и анализирует тип сообщения. Если сообщение является входом нового пользователя, он обновляет список пользователей и добавляет уведомление о новом пользователе онлайн в окно отображения чата; аналогично, если пользователь находится в автономном режиме, он удаляет пользователя из списка пользователей и отображает уведомление об автономном режиме в окне отображения чата; если это сообщение чата, он получает сообщение и отображает его в окне. Технологическая схема выглядит следующим образом.

![server](doc/pic/README/server.svg)

### Клиент

Во-первых, когда клиент входит в систему, он получает имя пользователя, номер группы, имя хоста и ip-адрес и передает их на сервер в локальной сети для обновления списка пользователей. Затем, когда клиенту нужно отправить сообщение, он вводит его в поле ввода чата и нажимает кнопку отправки, чтобы отправить чат, одновременно транслируя различные сообщения из локальной системы. Технологическая схема выглядит следующим образом:
...
***Отправляющая сторона, т.е. операция, которая берет на себя роль сервера.***

Нажмите на кнопку Отправить, чтобы открыть интерфейс Отправителя, нажмите на кнопку Открыть, выберите файл для отправки на локальном компьютере, нажмите на кнопку Отправить, и индикатор выполнения покажет информацию о текущей передаче файла, включая информацию о размере передаваемого файла, скорости передачи и так далее. Если вы хотите закрыть процесс отправки, нажмите кнопку Закрыть. Технологическая схема выглядит следующим образом:

![tcp_sender](doc/pic/README/tcp_sender.svg)

### Сторона приема

***Сторона получателя, т.е. действие, которое берет на себя роль клиента:***

Когда в главном интерфейсе появляется диалоговое окно с вопросом, принимать ли сообщение о передаче файла от пользователя, нажмите кнопку "Yes", если вы принимаете его, в противном случае нажмите кнопку "No". При приеме файла выберите каталог и имя файла, в котором хранится файл, и начните прием файла, в процессе также будет отображаться информация, такая как размер принятого файла, скорость приема и размер оставшегося времени. Технологическая схема выглядит следующим образом.
