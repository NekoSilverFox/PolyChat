...
Были применены несколько техник тест-дизайна: 
...
| 26   | mt_chatlist_getRandomPort      | ChatList         | Генерирует случайные порты между PORT_MIN и PORT_MAX.        |
| 27   | mt_chatlist_setChatState       | ChatList         | Окно чата открыто или закрыто.                               |
...
| 84   |                                    |                  |                                                              |
| 85   |                                    |                  |                                                              |
| 86   | mt_chatlist_btnchat_exist          | ChatList         | ui интерфейс обновляется корректно при добавлении нового группового чата. |
| 87   | mt_chatbox_userjoin_list           | ChatBox          | Пользователь присоединился (список пользователей обновляется корректно). |
| 88   | mt_chatbox_userjoin_counter        | ChatBox          | Пользователь присоединился (счетчик правильно увеличивается и ui отображается в правильном формате и с правильным содержанием). |
| 89   | mt_chatbox_userjoin_msgTextBrowser | ChatBox          | Пользователь присоединился (msgTextBrowser обновляется).     |
| 90   | mt_chatbox_userleft_list           | ChatBox          | Пользователь вышел (список пользователей обновляется корректно). |
| 91   | mt_chatbox_userleft_counter        | ChatBox          | Пользователь вышел (счетчик правильно увеличивается и ui отображается в правильном формате и с правильным содержанием). |
| 92   | mt_chatbox_userleft_msgTextBrowser | ChatBox          | Пользователь вышел (msgTextBrowser обновляется).             |
| 93   | mt_chatbox_clean                   | ChatBox          | Предупреждающее сообщение при очистке окна чата.             |
| 94   | mt_chatbox_save_empty              | ChatBox          | Имитация нажатия симулированным пользователем кнопки "Save", когда в пустом поле чата нет содержимого (с предупреждающим сообщением). |
| 95   | mt_chatbox_send_success            | ChatBox          | Имитация ввода пользователем текста в поле чата с помощью клавиатуры и последующее нажатие кнопки отправки. Введенная информация правильно отображается в сводке окна сообщения (сообщение успешно отправлено). |
| 96   | mt_tcpclient                       | TcpClient        | Все элементы управления интерфейса ui инициализируются правильным содержимым через интерфейс. |
| 97   | mt_tcpserver                       | TcpServer        | Все элементы управления интерфейса ui инициализируются правильным содержимым через интерфейс. |

**Результаты интеграционных тестирований:**

![image-20230218154016123](doc/pic/image-20230218154016123.png)

## Системное/End-to-End и другое тестирование
...
| ---- | ---------------------------- | ------------------------------------------------------------ | ----------------- | ------------------------------------------------------------ |
| 96   | e2e_Login_normal             | E2E                                                          | Login             | 1. Имитация открытия программы пользователем<br />2. Щелкните мышью на поле ввода имени пользователя и введите содержимое с помощью клавиатуры<br />3. Щелкните мышью на поле ввода номера группы и введите содержимое с помощью клавиатуры<br />4. Щелкните мышью на кнопке входа в систему |
| 97   | e2e_Login_empty_all          | E2E                                                          | Login             | 1. Имитация открытия программы пользователем<br />2. Щелкните мышью на кнопке входа в систему |
| 98   | e2e_Login_empty_name         | E2E                                                          | Login             | 1. Имитация открытия программы пользователем<br />2. Щелкните мышью на поле ввода имени пользователя и введите содержимое с помощью клавиатуры <br />3. Щелкните мышью на кнопке входа в систему |
| 99   | e2e_Login_empty_group        | E2E                                                          | Login             | 1. Имитация открытия программы пользователем<br />2. Щелкните мышью на поле ввода номера группы и введите содержимое с помощью клавиатуры<br />3. Щелкните мышью на кнопке входа в систему |
| 100  | e2e_ChatList_AddChat         | E2E                                                          | ChatList/Add Chat | 1. Доступ пользователя к главному окну（ChatList）<br />2. Щелкните мышью на кнопке "Добавить новый групповой чат".<br />3. Используйте клавиатуру для ввода текста в появившемся диалоговом окне 'Add chat'<br />4. Нажмите кнопку Подтвердить, чтобы добавить новый групповой чат |
| 101  | e2e_ChatList_search          | E2E                                                          | ChatList          | 1. Доступ пользователя к главному окну（ChatList）<br />2. Щелкните мышью на поле поиска<br />3. С помощью клавиатуры введите название группового чата, который вы хотите найти<br />4. Удаление ряда символов с помощью клавиатуры |
| 102  | e2e_ChatBox_normal_send      | E2E                                                          | ChatBox           | 1. Пользователь входит в групповой чат<br />2. Щелкните мышью на поле ввода текста<br />3. Введите текст<br />4. Введите текст<br />5. Пользователи нажимают на кнопку Выйти из группового чата |
| 103  | e2e_ChatBox_btnBold          | E2E                                                          | ChatBox           | 1. Пользователь входит в групповой чат<br />2. Щелкните мышью на поле ввода текста<br />3. Введите текст<br />4. Нажмите на кнопку полужирного текста<br />5. Введите текст<br />6. Введите текст<br />5. Пользователи нажимают на кнопку Выйти из группового чата |
| 104  | e2e_ChatBox_btnItalic        | E2E                                                          | ChatBox           | 1. Пользователь входит в групповой чат<br />2. Щелкните мышью на поле ввода текста<br />3. Введите текст<br />4. Нажмите на кнопку наклона текста<br />5. Введите текст<br />6. Введите текст<br />5. Пользователи нажимают на кнопку Выйти из группового чата |
| 105  | e2e_ChatBox_btnUnderLine     | E2E                                                          | ChatBox           | 1. Пользователь входит в групповой чат<br />2. Щелкните мышью на поле ввода текста<br />3. Введите текст<br />4. Нажмите на кнопку подчеркивания текста<br />5. Введите текст<br />6. Введите текст<br />5. Пользователи нажимают на кнопку Выйти из группового чата |
| 106  | e2e_ChatBox_other_user_left  | E2E                                                          | ChatBox           | 1. Пользователь входит в групповой чат<br />2. Доступ к групповому чату для других пользователей<br />3. Щелкните мышью на поле ввода текста<br />4. Введите текст<br />5. Введите текст<br />6. Другие пользователи оставляют<br />7. Пользователи нажимают на кнопку Выйти из группового чата |
| 107  | e2e_ChatBox_join_left        | E2E                                                          | ChatBox           | 1. Пользователи создают групповые чаты и присоединяются к ним<br />2. Пользователь 2 Присоединиться к групповому чату<br />3. Пользователь 3 Присоединиться к групповому чату<br />4. Пользователь 2 покидает групповой чат<br />5. Пользователи нажимают на кнопку Выйти из группового чата |
| 108  | e2e_TcpClient                | E2E                                                          | TcpClient         | 1. Пользователь вызывает окно TcpClient (указывая порт, размер файла, путь к файлу)<br />2. Период ожидания (время, необходимое для передачи файла в режиме)<br />3. Выход из транспортной программы TCP |
| 109  | e2e_TcpServer                | E2E                                                          | TcpServer         | 1. Пользователь вызывает окно TcpServer (указывая порт, размер файла, путь к файлу)<br />2. Период ожидания (время, необходимое для передачи файла в режиме)<br />3. Выход из приемника TCP |
| 110  | pt_Login_load                | Тестирование производительности                              | Login             | Производительность загрузки/вызова окон.                     |
| 111  | pt_AddChat_load              | Тестирование производительности                              | Add Chat          | Производительность загрузки/вызова окон.                     |
| 112  | pt_ChatList_load             | Тестирование производительности                              | ChatList          | Производительность загрузки/вызова окон.                     |
| 113  | pt_TcpClient_load            | Тестирование производительности                              | TcpClient         | Производительность загрузки/вызова окон.                     |
| 114  | pt_TcpServer_load            | Тестирование производительности                              | ChatBox           | Производительность загрузки/вызова окон.                     |
| 115  | pt_ChatBox_load              | Тестирование производительности                              | ChatBox           | Производительность загрузки/вызова окон.                     |
| 116  | pt_ChatBox_userjoin          | Тестирование производительности                              | ChatBox           | Пользователи входят в групповой чат.                         |
| 117  | pt_ChatBox_userjoin_left     | Тестирование производительности                              | ChatBox           | Пользователи входят в групповой чат и выходят из него вместе с другими пользователями. |
| 118  | pt_ChatBox_msgTextEdit_input | Тестирование производительности                              | ChatBox           | Имитация ввода пользователем с клавиатуры 100 символов в msgTextEdit и последующее нажатие кнопки отправить. |
| 119  | pt_Login_to_system           | Тестирование производительности                              | Login/ChatList    | Пользователи вводят свое имя и номер группы с клавиатуры, а затем нажимают кнопку входа для доступа к системе (ChatList). |
| 120  | pt_AddChat_ui                | Тестирование производительности                              | Add Chat/ChatBox  | Добавление нового группового чата путем имитации ввода пользователем команды `Add Chat` и последующего нажатия кнопки Подтвердить (Тестирование производительности при создании нового окна группового чата). |
| 121  | lt_ChatBox_x100              | Нагрузочное тестирование                                     | ChatBox           | Пользователь вступает в 100 групповых чатов.                 |
| 122  | lt_ChatBox_200user           | Нагрузочное тестирование                                     | ChatBox           | Гарантированная возможность подключения 200 пользователей в один чат. |
| 123  | lt_ChatBox_2000char          | Нагрузочное тестирование                                     | ChatBox           | Пользователь отправляет 2000 символов.                       |
| 124  | lt_ChatBox_msg_change        | Нагрузочное тестирование                                     | ChatBox           | Пользователь вводит сообщение, а затем изменяет стиль шрифта (bold, italic). |
| 125  | lt_TcpServer_x10             | Нагрузочное тестирование                                     | TcpServer         | Пользователь вызывает 10 окон отправки файлов TcpServer (отправляет 10 файлов). |
| 126  | lt_TcpClient_x10             | Нагрузочное тестирование                                     | TcpClient         | Пользователь вызывает 10 окон отправки файлов TcpClient (получает 10 файлов). |
| 127  | ct_ChatBox_code_normal       | Тестирование на совместимость                                | ChatBox           | Имитация ввода пользователем английского, китайского и русского языков в msgTextEdit с клавиатуры. При этом ui отображается корректно, без искажений. |
| 128  | GitHub CI                    | Кросс-платформенное тестирование (автоматизированное тестирование) | ALL               | Гарантированная компиляция и работа на macOS и Windows.      |
| 129  | GitHub CI                    | Автоматическое тестирование                                  | ALL               | Автоматизированная компиляция и тестирование на macOS и Windows. |
| 130  | GitHub CI                    | Тестирование на совместимость                                | ALL               | Скомпилируйте в Qt6.2.2 и убедитесь, что минимальная версия для macOS - 10.12; минимальная версия для Windows - 2019. |
...
| Системное/End-to-End тестирование | 14         |
